...
class WarningsAndErrorsTest {
    ...
    fun `project with modules - warn if Java version is not pinned`() {
        ...
        assertThat(result.output)
            .contains(
                """
                WARN: No 'jdk' version pinned (using: 17.0.13)
                 - Add jdk=17.0.13 to gradle/toolchain-versions.properties
        """
                    .trimIndent()
            )
    }
    ...
    fun `project with modules - warn if pinned Java version is not used`() {
        ...
        assertThat(result.output)
            .contains(
                """
                WARN: Gradle runs with Java 17.0.13. This project works best running with Java 17.0.99. 
