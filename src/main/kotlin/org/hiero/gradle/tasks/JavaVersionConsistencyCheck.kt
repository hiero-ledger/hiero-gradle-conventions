...
abstract class JavaVersionConsistencyCheck : DefaultTask() {
    ...
    /**
     * List of versions to ignore. This may be needed if versions for components that are not part
     * of the runtime module path of the applications are managed. For example, versions of
     * additional tools such as the protobuf compiler.
     */
        ...
        definedVersions.get().forEach { (id, version) ->
            val resolved =
                aggregatedClasspath.get().find {
                    ...
                    resolvedId is ModuleComponentIdentifier &&
                        resolvedId.moduleIdentifier.toString() == id
                }
            if (resolved == null) {
